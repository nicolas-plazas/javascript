    ...
    */
    if (xhttp.readyState === 4) {
      ...
      if (xhttp.status === 200) {
        ...
        var error = new Error("Error code: ".concat(xhttp.status, " - ").concat(urlApi)); // Regresa el callback con el error, creado anteriormente
...
fetchData(API, function (error1, data1) {
  ...
  fetchData("".concat(API).concat(data1.results[0].id), function (error2, data2) {
