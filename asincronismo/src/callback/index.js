...
// Ejemplo 2 - Obtener una fecha con timeOut

// Recibe como parametro una función, en este caso "printDate", para imprimir la fecha por consola
function date(callback) {
    console.log(new Date());
    // Función que refleja el contenido, luego de pasar 3 seg
    setTimeout(() => {
        // Variable que almacena la fecha actual
        let date = new Date();
        // llama a la función pasada por parámetro y le entrega la fecha obtenida en la varianle "date"
        callback(date);
    }, 3000);
}
